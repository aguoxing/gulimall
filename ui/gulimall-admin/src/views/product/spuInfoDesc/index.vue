...
<script>
import { listSpuInfoDesc, getSpuInfoDesc, delSpuInfoDesc, addSpuInfoDesc, updateSpuInfoDesc } from "@/api/product/spuInfoDesc";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSpuInfoDesc(this.queryParams).then(response => {
        this.spuInfoDescList = response.rows;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('product/spuInfoDesc/export', {
        ...this.queryParams
      }, `spuInfoDesc_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
