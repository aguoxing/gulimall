    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['product:skuSaleAttrValue:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['product:skuSaleAttrValue:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['product:skuSaleAttrValue:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['product:skuSaleAttrValue:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listSkuSaleAttrValue, getSkuSaleAttrValue, delSkuSaleAttrValue, addSkuSaleAttrValue, updateSkuSaleAttrValue } from "@/api/product/skuSaleAttrValue";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSkuSaleAttrValue(this.queryParams).then(response => {
        this.skuSaleAttrValueList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getSkuSaleAttrValue(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateSkuSaleAttrValue(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addSkuSaleAttrValue(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除sku销售属性&值编号为"' + ids + '"的数据项？').then(function() {
        return delSkuSaleAttrValue(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('product/skuSaleAttrValue/export', {
        ...this.queryParams
