    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['product:productAttrValue:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['product:productAttrValue:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['product:productAttrValue:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['product:productAttrValue:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listProductAttrValue, getProductAttrValue, delProductAttrValue, addProductAttrValue, updateProductAttrValue } from "@/api/product/productAttrValue";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listProductAttrValue(this.queryParams).then(response => {
        this.productAttrValueList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            ...
            addProductAttrValue(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除spu属性值编号为"' + ids + '"的数据项？').then(function() {
        return delProductAttrValue(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('product/productAttrValue/export', {
        ...this.queryParams
