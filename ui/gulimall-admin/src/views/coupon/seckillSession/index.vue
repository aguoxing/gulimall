...
<script>
import { listSeckillSession, getSeckillSession, delSeckillSession, addSeckillSession, updateSeckillSession } from "@/api/coupon/seckillSession";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSeckillSession(this.queryParams).then(response => {
        this.seckillSessionList = response.rows;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('coupon/seckillSession/export', {
        ...this.queryParams
      }, `seckillSession_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
