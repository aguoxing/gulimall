...
<script>
import { listHomeAdv, getHomeAdv, delHomeAdv, addHomeAdv, updateHomeAdv } from "@/api/coupon/homeAdv";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listHomeAdv(this.queryParams).then(response => {
        this.homeAdvList = response.rows;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('coupon/homeAdv/export', {
        ...this.queryParams
      }, `homeAdv_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
