...
<script>
import { listMemberStatisticsInfo, getMemberStatisticsInfo, delMemberStatisticsInfo, addMemberStatisticsInfo, updateMemberStatisticsInfo } from "@/api/member/memberStatisticsInfo";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listMemberStatisticsInfo(this.queryParams).then(response => {
        this.memberStatisticsInfoList = response.rows;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('member/memberStatisticsInfo/export', {
        ...this.queryParams
      }, `memberStatisticsInfo_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
