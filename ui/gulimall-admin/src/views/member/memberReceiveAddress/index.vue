...
<script>
import { listMemberReceiveAddress, getMemberReceiveAddress, delMemberReceiveAddress, addMemberReceiveAddress, updateMemberReceiveAddress } from "@/api/member/memberReceiveAddress";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listMemberReceiveAddress(this.queryParams).then(response => {
        this.memberReceiveAddressList = response.rows;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('member/memberReceiveAddress/export', {
        ...this.queryParams
      }, `memberReceiveAddress_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
