...
<script>
import { listGrowthChangeHistory, getGrowthChangeHistory, delGrowthChangeHistory, addGrowthChangeHistory, updateGrowthChangeHistory } from "@/api/member/growthChangeHistory";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listGrowthChangeHistory(this.queryParams).then(response => {
        this.growthChangeHistoryList = response.rows;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('member/growthChangeHistory/export', {
        ...this.queryParams
      }, `growthChangeHistory_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
