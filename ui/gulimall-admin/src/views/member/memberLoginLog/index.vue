    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['member:memberLoginLog:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['member:memberLoginLog:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['member:memberLoginLog:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listMemberLoginLog, getMemberLoginLog, delMemberLoginLog, addMemberLoginLog, updateMemberLoginLog } from "@/api/member/memberLoginLog";
  ...
  methods: {
    ...
    getList() {
      ...
      listMemberLoginLog(this.queryParams).then(response => {
        this.memberLoginLogList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getMemberLoginLog(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateMemberLoginLog(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addMemberLoginLog(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('member/memberLoginLog/export', {
        ...this.queryParams
