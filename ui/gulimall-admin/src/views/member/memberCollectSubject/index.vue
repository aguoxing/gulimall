<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="memberCollectSubjectList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="subject_img" align="center" prop="subjectImg" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="subject_img" prop="subjectImg">
          <el-input v-model="form.subjectImg" type="textarea" placeholder="请输入内容" />
        </el-form-item>
...
<script>
import { listMemberCollectSubject, getMemberCollectSubject, delMemberCollectSubject, addMemberCollectSubject, updateMemberCollectSubject } from "@/api/member/memberCollectSubject";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listMemberCollectSubject(this.queryParams).then(response => {
        this.memberCollectSubjectList = response.rows;
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除会员收藏的专题活动编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('member/memberCollectSubject/export', {
        ...this.queryParams
      }, `memberCollectSubject_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
