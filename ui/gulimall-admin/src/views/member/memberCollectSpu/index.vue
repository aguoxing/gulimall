...
<script>
import { listMemberCollectSpu, getMemberCollectSpu, delMemberCollectSpu, addMemberCollectSpu, updateMemberCollectSpu } from "@/api/member/memberCollectSpu";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listMemberCollectSpu(this.queryParams).then(response => {
        this.memberCollectSpuList = response.rows;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('member/memberCollectSpu/export', {
        ...this.queryParams
      }, `memberCollectSpu_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
