...
<script>
import { listWareOrderTask, getWareOrderTask, delWareOrderTask, addWareOrderTask, updateWareOrderTask } from "@/api/ware/wareOrderTask";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listWareOrderTask(this.queryParams).then(response => {
        this.wareOrderTaskList = response.rows;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('ware/wareOrderTask/export', {
        ...this.queryParams
      }, `wareOrderTask_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
